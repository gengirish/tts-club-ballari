...
/** PNGs consumed by docs/demo-doc-print.html → npm run docs:demo-doc-pdf */
const OUT_DIR = path.join(process.cwd(), "docs", "demo-screenshots");
...
test.describe("Product demo PDF — public pages (safe on production)", () => {
...
test.describe("Product demo PDF — signed-in member app (staging/local)", () => {
  test("captures main member journeys for the printable demo", async ({ page }) => {
    ...
    await page.screenshot({ path: shot("18-app-help.png"), fullPage: true });
  });
});
